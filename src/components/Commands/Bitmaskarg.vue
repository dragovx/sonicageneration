...
<div class="bitmaskarg1_table" v-show="bitmaskarg.window" v-click-outside="onClickOutside">
      <table style="background-color: gray; color: white">
        ...
        <tr v-for="elem in bitmaskarg.masvalue" :key="elem.id">
          ...
          <td><input type="checkbox" v-model="elem.value" @change="[elem.value ? bitmaskarg.value+=Math.pow(2,elem.position) : bitmaskarg.value-=Math.pow(2,elem.position)], submitForm(bitmaskarg)" :disabled="bitmaskarg.disabled"></td>
        </tr>
      </table>
    </div>
</div>
